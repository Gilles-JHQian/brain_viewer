...
export async function buildKdeFrameColorCache(
  ...
  const fixedRange = await estimateFixedDensityRangeAsync(
    influenceMap,
    hgaValuesByFrame,
    globalHgaMax,
    positions,
  ...
  onProgress?.(0, hgaValuesByFrame.length, fixedRange);
  ...
  for (let index = 0; index < order.length; index += chunkSize) {
    ...
    onProgress?.(done, hgaValuesByFrame.length, fixedRange);
    await idleYield(16);
  }

  return { cache, fixedRange };
}
