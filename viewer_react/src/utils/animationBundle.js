export function animationLoadKey(selectedLoad) {
  return selectedLoad === 'all' ? 'all' : `load${selectedLoad}`;
}
...
export function extractBundleForLoad(subjectPhasePayload, selectedLoad) {
  const loadKey = animationLoadKey(selectedLoad);
